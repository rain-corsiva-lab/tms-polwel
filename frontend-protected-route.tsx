...
export const ProtectedRoute: React.FC<ProtectedRouteProps> = ({ children, requiredRoles = [], fallbackPath = "/login", organizationId }) => {
      ...
      <div className="min-h-screen flex items-center justify-center bg-gray-50">
        <div className="max-w-md w-full bg-white shadow-lg rounded-lg p-6">
          <div className="text-center">
            ...
            <p className="mt-1 text-sm text-gray-500">Your account is {user.status.toLowerCase()}. Please contact an administrator.</p>
          </div>
        </div>
      </div>
    );
            ...
            <div className="mx-auto flex items-center justify-center h-12 w-12 rounded-full bg-red-100">
              <svg className="h-6 w-6 text-red-600" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                <path
                  ...
                  strokeLinejoin="round"
                  strokeWidth={2}
                  d="M19 21V5a2 2 0 00-2-2H7a2 2 0 00-2 2v16m14 0h2m-2 0h-2m-2 0H7m5 0v-5a2 2 0 00-2-2H7a2 2 0 00-2 2v5m5 0v-2a1 1 0 00-1-1h-1a1 1 0 00-1 1v2m5-10V7a2 2 0 00-2-2H7a2 2 0 00-2 2v3m10 0h-5m5 0h2"
                />
              </svg>
            </div>
            <h3 className="mt-2 text-sm font-medium text-gray-900">Organisation Access Denied</h3>
            <p className="mt-1 text-sm text-gray-500">You don't have access to this organisation's data.</p>
          </div>
        </div>
      </div>
    );
...
export const RoleBased: React.FC<RoleBasedProps> = ({ children, requiredRoles, fallback = null }) => {
