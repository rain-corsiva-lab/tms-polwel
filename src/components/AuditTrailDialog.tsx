          ...
          <ScrollArea className="h-[500px] pr-4">
            <div className="space-y-4">
              {!Array.isArray(auditTrail) || auditTrail.length === 0 ? (
                <div className="text-center py-8 text-muted-foreground">No audit trail entries found</div>
              ) : (
                    ...
                    <div className="flex items-start justify-between">
                      <div className="flex items-center gap-2">
                        ...
                        <span className="text-sm text-muted-foreground">
                          {`${formatDate(entry.timestamp)} ${new Date(entry.timestamp).toLocaleTimeString("en-GB", { hour: "2-digit", minute: "2-digit" })}`}
                        </span>
                      </div>
                    </div>
