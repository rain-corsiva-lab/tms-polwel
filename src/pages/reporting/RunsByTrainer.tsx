...
export default function RunsByTrainer() {
  ...
  const [filterOptions, setFilterOptions] = useState<FilterOptions>({ trainers: [], statuses: [] });
  ...
  const handleExportToExcel = async () => {
    try {
      ...
      const exportData = allRuns.map((run: CourseRun) => ({
        ...
        Trainers: run.trainers.map((t) => t.name).join(", "),
      ...
      XLSX.utils.book_append_sheet(wb, ws, "Runs by Trainer");
      XLSX.writeFile(wb, "Runs_by_Trainer.xlsx");
      ...
      <div className="container mx-auto py-6 space-y-6">
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-4">
            ...
            <div>
              <h1 className="text-3xl font-bold tracking-tight">Runs by Trainer</h1>
              <p className="text-muted-foreground">View all course runs grouped by trainer</p>
            </div>
          </div>
            ...
            {loading ? (
              ...
              <>
                <Table>
                  ...
                  <TableBody>
                    {runs.map((run) => {
                      const trainerNames = run.trainers && run.trainers.length > 0 ? run.trainers.map((t) => t.name).join(", ") : "N/A";
                      return (
                        <TableRow key={run.id}>
