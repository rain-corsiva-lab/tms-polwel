...
export async function generateBillingXLSX(courseRunId: string, courseRunCode: string) {
  ...
  worksheet.mergeCells('A2:Y2');
    ...
    const courseDates = courseRun.startDatetime
      ? `${new Date(courseRun.startDatetime).toLocaleDateString('en-GB', { timeZone: 'Asia/Singapore' })} - ${new Date(courseRun.endDatetime).toLocaleDateString('en-GB', { timeZone: 'Asia/Singapore' })}`
      : '';
